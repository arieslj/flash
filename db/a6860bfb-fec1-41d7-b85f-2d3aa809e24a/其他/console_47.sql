...
select
    ...
    ,f1.piece_name 片区
    ,f1.store_name 网点
    ,f1.store_non_delivery_and_non_call_ratio 该网点交接未妥投且未拨打电话包裹占比
    ,f1.store_non_delivery_and_only_one_call_ratio  该网点未妥投且拨打1次电话包裹占比
    ,f1.staff_info_id 员工ID
	,concat(round(f1.staff_non_delivery_and_non_call_ratio*100,2),'%') as 交接未妥投且未拨打电话包裹占比
	,concat(round(f1.staff_non_delivery_and_only_one_call_ratio *100,2),'%') as 交接未妥投且拨打1次电话包裹占比
	,f1.handover_cn 交接包裹量
	,f1.delivery_cn 交接且妥投包裹量
	,f1.delivery_non_returned_cn 交接包裹妥投包裹量_剔除退件妥投
	,f1.returned_cn 交接包裹退件包裹量
	,f1.innormal_cn 交接包裹异常关闭包裹量
	,f1.handover_unfinished_cn 交接包裹未终态包裹量
	,f1.non_delivery_and_non_call_cn 交接未妥投且未拨打电话包裹量
	,f1.non_delivery_and_only_one_call_cn 交接未妥投且拨打电话1次包裹量
	,f1.non_delivery_and_only_two_call_cn 交接未妥投且拨打电话2次包裹量
	,f1.non_delivery_and_only_three_call_cn 交接未妥投且拨打电话3次包裹量
	,f1.non_delivery_and_only_above_three_call_cn 交接未妥投且拨打电话3次以上包裹量
from tmpale.tmp_handover_call_ratio_fn f1
where f1.stat_date=current_date
;
...
select
    f3.stat_date as 日期
    ,f6.交接包裹快递员数
    ,f3.交接未妥投且未拨打电话包裹占比 as 交接未妥投且未拨打电话包裹占比

	,f6.`交接未妥投且未拨打电话包裹占比大于10%_快递员占比`
	# ,f6.`交接且未拨打电话包裹占比大于20%_快递员占比`
	# ,f6.`交接且未拨打电话包裹占比大于30%_快递员占比`
	# ,f6.`交接且未拨打电话包裹占比大于40%_快递员占比`
   ,f6.`交接未妥投且未拨打电话包裹占比大于50%_快递员占比`
from
	( #全网今日
		select
		    f1.stat_date
			,sum(f1.non_delivery_and_non_call_cn)/sum(f1.handover_cn) as 交接未妥投且未拨打电话包裹占比
			,sum(f1.non_delivery_and_only_one_call_cn)/sum(f1.handover_cn) as 交接未妥投且拨打电话一次包裹占比
		from tmpale.tmp_handover_call_ratio_fn f1
		where f1.stat_date>= date_sub(current_date,interval 7 day)
            and f1.stat_date>'2023-10-19'
		group by 1
	)f3
left join
	(
		select
		    f1.stat_date
		    ,count(distinct f1.staff_info_id) as 交接包裹快递员数
			,count( distinct if(f1.staff_non_delivery_and_non_call_ratio+0>0.1,f1.staff_info_id,null))/count(distinct f1.staff_info_id) as '交接未妥投且未拨打电话包裹占比大于10%_快递员占比'
		   # ,count( distinct if(f1.staff_non_delivery_and_non_call_ratio+0>0.2,f1.staff_info_id,null))/count(distinct f1.staff_info_id) as '交接未妥投且未拨打电话包裹占比大于20%_快递员占比'
		   # ,count( distinct if(f1.staff_non_delivery_and_non_call_ratio+0>0.3,f1.staff_info_id,null))/count(distinct f1.staff_info_id) as '交接未妥投且未拨打电话包裹占比大于30%_快递员占比'
		   # ,count( distinct if(f1.staff_non_delivery_and_non_call_ratio+0>0.4,f1.staff_info_id,null))/count(distinct f1.staff_info_id) as '交接未妥投且未拨打电话包裹占比大于40%_快递员占比'
			,count( distinct if(f1.staff_non_delivery_and_non_call_ratio+0>0.5,f1.staff_info_id,null))/count(distinct f1.staff_info_id) as '交接未妥投且未拨打电话包裹占比大于50%_快递员占比'
			,count( distinct if(f1.staff_non_delivery_and_only_one_call_ratio+0>0.06,f1.staff_info_id,null))/count(distinct f1.staff_info_id) as '交接未妥投且拨打电话一次包裹占比大于6%_快递员占比'
			,count( distinct if(f1.staff_non_delivery_and_only_one_call_ratio+0>0.12,f1.staff_info_id,null))/count(distinct f1.staff_info_id) as '交接未妥投且拨打电话一次包裹占比大于12%_快递员占比'
		from tmpale.tmp_handover_call_ratio_fn f1
		where f1.stat_date>= date_sub(current_date,interval 7 day)
            and f1.stat_date>'2023-10-19'
		group by 1
	)f6 on f3.stat_date=f6.stat_date
   order by 1 desc
 ;
...
left join
	(
		...
		where f1.stat_date= current_date
		group by 1,2
	)f6 on f3.region_name=f6.region_name
order by 4 desc
;
...
left join
	(
		select
		   ...
		   # ,count( distinct if(f1.staff_non_delivery_and_non_call_ratio+0>0.4,f1.staff_info_id,null))/count(distinct f1.staff_info_id) as '交接未妥投且未拨打电话包裹占比大于40%_快递员占比'
			,count( distinct if(f1.staff_non_delivery_and_non_call_ratio+0>0.5,f1.staff_info_id,null))/count(distinct f1.staff_info_id) as '交接未妥投且未拨打电话包裹占比大于50%_快递员占比'
			,count( distinct if(f1.staff_non_delivery_and_only_one_call_ratio+0>0.06,f1.staff_info_id,null))/count(distinct f1.staff_info_id) as '交接未妥投接且拨打电话一次占包裹比大于6%_快递员占比'
			,count( distinct if(f1.staff_non_delivery_and_only_one_call_ratio+0>0.12,f1.staff_info_id,null))/count(distinct f1.staff_info_id) as '交接未妥投且拨打电话一次包裹占比大于12%_快递员占比'
		from tmpale.tmp_handover_call_ratio_fn f1
		where f1.stat_date= current_date
		group by 1,2,3,4
	)f6 on f3.store_name=f6.store_name
order by 6 desc
;
...
select
    ...
    ,f1.piece_name 片区
    ,f1.store_name 网点
    ,f1.store_non_delivery_and_non_call_ratio 该网点交接未妥投且未拨打电话包裹占比_20231102
    ,f1.staff_info_id 员工ID
	,f1.handover_cn 交接量_20231110
	,f1.delivery_cn 交接包裹妥投量_20231110
	,concat(round(f1.staff_non_delivery_and_non_call_ratio*100,2),'%') as 交接未妥投且未拨打电话包裹占比_20231110
	,concat(round(f2.staff_non_delivery_and_non_call_ratio*100,2),'%') as 交接未妥投且未拨打电话包裹占比_20231109
	,concat(round(f3.staff_non_delivery_and_non_call_ratio*100,2),'%') as 交接未妥投且未拨打电话包裹占比_20231108

from tmpale.tmp_handover_call_ratio_fn f1
left join tmpale.tmp_handover_call_ratio_fn f2 on f1.staff_info_id=f2.staff_info_id and f2.stat_date=date_sub(current_date,interval 1 day)
left join tmpale.tmp_handover_call_ratio_fn f3 on f1.staff_info_id=f3.staff_info_id and f3.stat_date=date_sub(current_date,interval 2 day)
where f1.stat_date=current_date
and f1.staff_non_delivery_and_non_call_ratio>0.5
and f2.staff_non_delivery_and_non_call_ratio>0.5
and f3.staff_non_delivery_and_non_call_ratio>0.5
