...
(
    ...
    left join my_backyard.staff_work_attendance swa on hsa.sub_staff_info_id = swa.staff_info_id and swa.attendance_date = '${date}' and swa.organization_id = hsa.store_id and ( swa.started_at is not null  or swa.end_at is not null )
    where
        hsa.actual_begin_date <= '${date}'
        and coalesce(hsa.actual_end_date, curdate()) >= '${date}'
        and hsa.employment_begin_date <= '${date}'
...
left join
    (
        ...
        join my_backyard.staff_work_attendance swa on swa.staff_info_id = hsi.staff_info_id and swa.attendance_date = '${date}' and ( swa.started_at is not null or swa.end_at is not null)
...
left join
    (
        ...
        from dwm.dwd_my_dc_should_be_delivery ds
        left join my_staging.parcel_route pr on pr.pno = ds.pno and pr.route_action = 'SORTING_SCAN' and pr.routed_at >= date_sub('${date}', interval 8 hour) and pr.routed_at < date_add('${date}', interval 16 hour)
        group by 1
...
(
    ...
    from
        (
            ...
            join my_staging.ticket_delivery td on td.pno = a1.pno and td.created_at >= date_sub('${date}', interval 8 hour) and td.created_at < date_add('${date}', interval 16 hour)
...
left join
    (
        ...
        left join my_backyard.staff_work_attendance swa on swa.staff_info_id = s1.sub_staff_info_id and swa.attendance_date = '${date}' and swa.organization_id = s1.store_id
        left join
            (
                select
                    t1.staff_info_id
        ...
        left join
            (
                ...
                where
                    pi.created_at >= date_sub('${date}', interval 8 hour)
                    and pi.created_at < date_add('${date}', interval 16 hour)
                group by 1
            ...
            (
                ...
                from
                    (
                        ...
                        from total t
                        join my_staging.parcel_route pr on pr.pno = t.pno and pr.route_action = 'PHONE' and pr.routed_at >= date_sub('${date}', interval 8 hour) and pr.routed_at < date_add('${date}', interval 16 hour)
                        where
                            t.state != 5
                        group by 1,2
                    ) a
