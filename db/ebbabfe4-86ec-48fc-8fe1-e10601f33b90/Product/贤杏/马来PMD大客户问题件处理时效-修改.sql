...
(
    ...
    left join
        ...
        ) a on a.client_id = cdt.client_id
    where cdt.organization_type = 2
   ...
   -- and a.client_id  is null
    and if(cdt.client_id = 'AA0105' and di.diff_marker_category in (22,20,21,32,26,31,120), 1 = 1, a.client_id is null)
   -- and di.pno = 'M1403WMQEKAA'
)
...
select
    ...
    , tt.before_12总量 before_12总量_tt
    , tt.before_12已处理量 before_12已处理量_tt
    , tt.before_12处理率 before_12处理率_tt
    , tt.before_12及时量 before_12及时量_tt
    , tt.before_12及时率 before_12及时率_tt
    , tt.before_12未处理量 before_12未处理量_tt
    , tt.should_today总量 should_today总量_tt
    , tt.should_today已处理量 should_today已处理量_tt
    , tt.should_today处理率 should_today处理率_tt
    , tt.should_today及时量 should_today及时量_tt
    , tt.should_today及时率 should_today及时率_tt
    , tt.should_today未处理量 should_today未处理量_tt
    , tt.退货量 退货量_tt
    , tt.继续派送量 继续派送量_tt
    , tt.丢弃量 丢弃量_tt
    , tt.理赔数量 理赔数量_tt

    , laz.before_12总量 before_12总量_laz
    , laz.before_12已处理量 before_12已处理量_laz
    , laz.before_12及时量 before_12及时量_laz
    , laz.before_12未处理量 before_12未处理量_laz
    , laz.should_today总量 should_today总量_laz
    , laz.should_today已处理量 should_today已处理量_laz
    , laz.should_today及时量 should_today及时量_laz
    , laz.should_today未处理量 should_today未处理量_laz
    , laz.before_12处理率 before_12处理率_laz
    , laz.before_12及时率 before_12及时率_laz
    , laz.should_today处理率 should_today处理率_laz
    , laz.should_today及时率 should_today及时率_laz
    , laz.退货量 退货量_laz
    , laz.继续派送量 继续派送量_laz
    , laz.丢弃量 丢弃量_laz
    , laz.理赔数量 理赔数量_laz

    , sho.before_12总量 before_12总量_sho
    , sho.before_12已处理量 before_12已处理量_sho
    , sho.before_12及时量 before_12及时量_sho
    , sho.before_12未处理量 before_12未处理量_sho
    , sho.should_today总量 should_today总量_sho
    , sho.should_today已处理量 should_today已处理量_sho
    , sho.should_today及时量 should_today及时量_sho
    , sho.should_today未处理量 should_today未处理量_sho
    , sho.before_12处理率 before_12处理率_sho
    , sho.before_12及时率 before_12及时率_sho
    , sho.should_today处理率 should_today处理率_sho
    , sho.should_today及时率 should_today及时率_sho
    , sho.退货量 退货量_sho
    , sho.继续派送量 继续派送量_sho
    , sho.丢弃量 丢弃量_sho
    , sho.理赔数量 理赔数量_sho

    , she.before_12总量 before_12总量_she
    , she.before_12已处理量 before_12已处理量_she
    , she.before_12及时量 before_12及时量_she
    , she.before_12未处理量 before_12未处理量_she
    , she.should_today总量 sheuld_today总量_she
    , she.should_today已处理量 sheuld_today已处理量_she
    , she.should_today及时量 sheuld_today及时量_she
    , she.should_today未处理量 sheuld_today未处理量_she
    , she.before_12处理率 before_12处理率_she
    , she.before_12及时率 before_12及时率_she
    , she.should_today处理率 sheuld_today处理率_she
    , she.should_today及时率 sheuld_today及时率_she
    , she.退货量 退货量_she
    , she.继续派送量 继续派送量_she
    , she.丢弃量 丢弃量_she
    , she.理赔数量 理赔数量_she
...
left join
    (
        ...
        from t t1
        where
            t1.client_name = 'lazada'
        group by 1,2
        order by 1,2
    ) laz on laz.date_d = total.date_d and laz.CN_element = total.CN_element
left join
    (
        select
             date(t1.should_deal_time) date_d
        ...
        from t t1
        where
            t1.client_name = 'tiktok'
        group by 1,2
        order by 1,2
    ) tt on tt.date_d = total.date_d and tt.CN_element = total.CN_element
left join
    (
        select
             date(t1.should_deal_time) date_d
        ...
        from t t1
        where
            t1.client_name = 'shopee'
        group by 1,2
        order by 1,2
    ) sho on sho.date_d = total.date_d and sho.CN_element = total.CN_element
left join
    (
        select
             date(t1.should_deal_time) date_d
        ...
        from t t1
        where
            t1.client_name = 'shein'
        group by 1,2
        order by 1,2
    ) she on she.date_d = total.date_d and she.CN_element = total.CN_element
order by 1,2
;
