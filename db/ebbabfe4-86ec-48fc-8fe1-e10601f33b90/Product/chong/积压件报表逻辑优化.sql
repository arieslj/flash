with t as
    (
        ...
        where
            pi.created_at >=convert_tz('${sdate}','+08:00','+00:00')
...
select
    ...
    ,case
        ...
        when t1.state<>6 and pr.store_name in ('Autoqaqc')  then '调度'
...
(
 ...
 from
   (
    select
      di.*
      ,row_number() over (partition by di.pno order by di.created_at desc ) rnk
...
(
  ...
  (
    ...
    and pr.pno_created_at>=convert_tz('${sdate}','+08:00','+00:00')
   )
   where rk=1
) pr on pr.pno = t1.pno
left join
...
from t t1
     ...
     (select pc.*,
      ...
      from (select *
            ...
            where cfg_key = 'diff.ticket.customer_service.ka_client_ids') pc lateral view explode(split(pc.cfg_value, ',')) id as client_id) cs
