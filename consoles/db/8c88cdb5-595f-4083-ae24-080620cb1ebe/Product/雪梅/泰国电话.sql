...
with t as
    (
        ...
        left join rot_pro.parcel_route pr on swa.staff_info_id = pr.staff_info_id  and pr.routed_at >= date_sub(date_sub(curdate(), interval 2 day), interval 7 hour) and pr.routed_at >= date_sub(swa.attendance_date, interval 7 hour) and pr.routed_at < date_add(swa.attendance_date, interval 17 hour) and pr.route_action in ('PHONE', 'INCOMING_CALL')
        left join fle_staging.parcel_info pi on pi.pno = pr.pno and pi.created_at >= date_sub(date_sub(curdate(), interval 2 day), interval 7 hour)
        left join bi_pro.hr_staff_info hsi on hsi.staff_info_id = swa.staff_info_id
        where
            swa.attendance_date >= date_sub(curdate(), interval 2 day)
            and swa.attendance_date <= date_sub(curdate(), interval 1 day)
...
select
    ...
    ,if(t1.incoming_count is not null, (coalesce(ci.incoming_count, 0) + coalesce(c2.incoming_count, 0))/coalesce(t1.incoming_count, 0), null) 有效接线率
    ,case
        ...
        when coalesce(t1.incoming_count, 0)  > 0 and if(t1.incoming_count is not null,(coalesce(ci.incoming_count, 0) + coalesce(c2.incoming_count, 0))/coalesce(t1.incoming_count, 0), null) < 0.5 then '异常'
        when coalesce(t1.incoming_count, 0)  > 0 and if(t1.incoming_count is not null,(coalesce(ci.incoming_count, 0) + coalesce(c2.incoming_count, 0))/coalesce(t1.incoming_count, 0), null) >= 0.5 then '否'
        else null
    end 是否异常
#     count(1)
    ,curdate() p_date
...
from
    (
        ...
        left join bi_pro.hr_staff_info hsi on hsi.staff_info_id = swa.staff_info_id
        where
            swa.stat_date >= date_sub(curdate(), interval 2 day)
            and swa.stat_date <= date_sub(curdate(), interval 1 day)
            and swa.job_title in (13,110,452)
...
left join
    (
        ...
        left join rot_pro.parcel_route pr on pr.routed_at > date_sub(curdate(), interval 10 day ) and pr.staff_info_id = a1.main_staff_info_id and pr.route_action in ( 'INCOMING_CALL') and pr.routed_at < date_add(a1.attendance_date, interval 7 hour) and pr.routed_at >= date_add(date_sub(a1.attendance_date, interval 7 day), interval 17 hour)
        left join rot_pro.parcel_route pr2 on pr2.routed_at > date_sub(curdate(), interval 10 day  ) and pr2.staff_info_id = hsa.sub_staff_info_id and pr2.route_action in ( 'INCOMING_CALL') and pr2.routed_at < date_add(a1.attendance_date, interval 7 hour) and pr2.routed_at >= date_add(date_sub(a1.attendance_date, interval 7 day), interval 17 hour)
        group by 1,2
...
select
    t.*
from tmpale.tmp_th_courier_phone_record t
where
    t.日期 >= '{sdate}'
    and t.日期 <= '${edate}'
