                    ...
                    (
                        ...
                        from
                            (
                                ...
                                where
                                    plt.parcel_created_at >= '${sdate}' and plt.parcel_created_at < date_add('${edate}', interval 1 day) and plt.state = 6 and plt.duty_result in (1,2) and plt.penalties > 0
                                ...
                                from my_bi.parcel_lose_task plt
                                left join my_staging.parcel_info pi on pi.pno = plt.pno and pi.created_at > date_sub('${sdate}', interval 2 month)
                                left join my_bi.parcel_lose_task plt2 on plt2.pno = pi.customary_pno and plt2.source = 11 and plt2.parcel_created_at > date_sub('${sdate}', interval 2 month)
                                where
...
#             ,pi2.insure_declare_value/100
        ...
        left join my_staging.order_info pai on pai.pno = if(pi.pno is null, t1.pno, pi2.pno)
#         where
#            pi.created_at >  date_sub('${sdate}', interval 3 month)
    )
, cla as
    (
        select
