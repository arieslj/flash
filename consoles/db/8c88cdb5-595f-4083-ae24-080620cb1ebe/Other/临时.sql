...
(
    ...
    left join backyard_pro.hr_staff_info hsi on hsa.staff_info_id=hsi.staff_info_id
    where
                ...
                (
                    ...
                    from
                        (
                            ...
                            join dwm.drds_parcel_sorting_code_info ps on ds.pno =  ps.pno and ds.store_id = ps.dst_store_id
                            where
                                ds.stat_date = '${date}'
                        ) a
                    where
...
select
    ...
    ,t2.大区, t2.片区, t2.来源网点, t2.主账号, t2.入职日期 ,t2.是否子账号 ,t2.子账号, t2.原岗位,t2.`van/bike`, t2.上班时间, t2.下班时间, t2.揽收量, t2.交接量, t2.妥投量, t2.派送时长, t2.交接三段码数量, t2.妥投三段码数量, t2.未妥投中打电话次数为0的数量, t2.未妥投中打电话次数为1的数量
from
    (
        select
            dt.region_name 大区
        ...
        left join
            (
                ...
                left join sup s1 on s1.store_id = ds.store_id and pi.ticket_delivery_staff_info_id = s1.sub_staff_info_id
                left join bi_pro.hr_staff_info hsi on hsi.staff_info_id = pi.ticket_delivery_staff_info_id and hsi.formal = 1 and hsi.is_sub_staff = 0 and hsi.sys_store_id = ds.store_id
                where
                    pi.state = 5
                    and ds.stat_date = '${date}'
                    and pi.finished_at >= date_sub('${date}', interval 7 hour)
                    and pi.finished_at < date_add('${date}', interval 17 hour)
                    and pi.returned = 0
                group by 1
            ) a5 on a5.store_id = a1.store_id
...
left join
    (
        select
            ...
            ,s1.hire_date 入职日期
            ,if(s1.is_sub_staff=1,'是','否') 是否子账号
...
from
    (
        ...
        join tmpale.tmp_th_pno_lj_0312 t on t.pno = pct.pno
...
from tmpale.tmp_th_pno_lj_0321 t
left join bi_pro.parcel_lose_task plt on plt.pno = t.pno and plt.state = 6 and plt.duty_result = 1
group by 1,2
