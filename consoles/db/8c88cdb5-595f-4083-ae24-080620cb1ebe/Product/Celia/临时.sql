...
select
    t.pno
    ,if(pi.returned = 1, '退件', '正向') lx
from fle_staging.parcel_info pi
join tmpale.tmp_th__pno_lj_0219 t on t.pno = pi.pno
;
...
join tmpale.tmp_th_staff_lj_0221 t on t.staff = hsi.staff_info_id
...
select
    ...
    ,convert_tz(ci.created_at, '+00:00', '+07:00')  进入时间
from fle_staging.customer_issue ci
join tmpale.tmp_th_pno_lj_0219 t on t.pno = ci.pno
where
    ci.request_sub_type in (221, 300)


;
select count(1) from tmpale.tmp_th_pno_lj_0219
;
select
    t.pno
    ,case pci.source
    ...
    ,pcip.created_at 第二次进入时间
from bi_center.parcel_complaint_inquiry pci
join tmpale.tmp_th_pno_lj_0219 t on t.pno = pci.merge_column
left join bi_center.parcel_complaint_inquiry_punish pcip on pcip.merge_column = t.pno
...
from tmpale.tmp_th_pno_staff_0223 t
left join fle_staging.parcel_info pi on pi.pno = t.pno
left join bi_pro.hr_staff_info hsi on hsi.staff_info_id = t.satff
;
COALESCE(CAST(pi.`ka_warehouse_id` as CHAR),concat(pi.client_id,'-',trim(pi.`src_phone`),'-',trim(pi.src_detail_address))) seller
...
select
    ...
    ,dt.store_name 目的地网点
    ,dt.piece_name 目的地片区
    ,dt.region_name 目的地大区
from tmpale.tmp_th_pno_lj_0626 t
left join fle_staging.parcel_info pi on pi.pno = t.pno
left join dwm.dim_th_sys_store_rd dt on dt.store_id = pi.dst_store_id and dt.stat_date = curdate()
...
select
    ...
    ,pi.ticket_delivery_staff_info_id 快递员ID
    ,pci.created_at 询问任务生成时间
    ,if(pci.merge_column is not null, '是', '否') 是否有丢失处罚
    ,if(ab.merge_column is not null, '是', '否') 是否有EPOD拍照不合格处罚
...
left join
    ( -- 丢失处罚
        select
            ...
            ,am.merge_column
        from bi_center.parcel_complaint_inquiry pci
        join tmpale.tmp_th_pno_lj_0809 t on t.pno = pci.merge_column
        left join bi_pro.abnormal_message am on json_extract(am.extra_info, '$.id') = pci.id and json_extract(am.extra_info, '$.src') = 'parcel_complaint_inquiry' and am.punish_category = 7
        where
            pci.created_at > date_sub(curdate(), interval 3 month)
    ) pci on pci.pno = t.pno
...
left join
    (
        ...
        join tmpale.tmp_th_pno_lj_0809 t on t.pno = am.merge_column
        where
            am.abnormal_time > date_sub(curdate(), interval 3 month)
            and am.punish_category = 80
    ) ab on ab.merge_column = t.pno
...
select
    ...
    ,sd.name 所属部门
from fle_staging.parcel_info pi
join tmpale.tmp_th_pno_lj_1010 t on t.pno = pi.pno
left join fle_staging.ka_profile kp on kp.id = pi.client_id
