...
select
    t1.client_name
    -- 超时效丢失
    ,count(distinct if(t1.result_rank = 3 and plt.pno is not null, t1.pno, null)) 超时效_丢失_票数
    ,sum(if(t1.result_rank = 3 and plt.pno is not null and pi2.state = 8, coalesce(c1.claim_value, pc.claim_money, v1.parcel_value), 0)) 超时效_丢失_预估金额
    ,sum(if(t1.result_rank = 3 and plt.pno is not null and pi2.state = 8, coalesce(c1.claim_value, pc.claim_money), 0)) 超时效_丢失_理赔金额
    -- 拍卖仓妥投
    ,count(distinct if(t1.result_rank = 3 and plt.pno is null and pi2.state = 8, t1.pno, null)) 超时效_拍卖仓妥投_票数
    ,sum(if(t1.result_rank = 3 and plt.pno is null and pi2.state = 8, coalesce(c1.claim_value, pc.claim_money, v1.parcel_value), 0)) 超时效_拍卖仓妥投_预估金额
    ,sum(if(t1.result_rank = 3 and plt.pno is null and pi2.state = 8, coalesce(c1.claim_value, pc.claim_money), 0)) 超时效_拍卖仓妥投_理赔金额
    --
    ,count(distinct if(t1.result_rank = 3 and pi2.state = 5 and pi2.cod_enabled = 1, t1.pno, null)) 超时效_COD妥投_票数
    ,sum(if(t1.result_rank = 3 and pi2.state = 5 and pi2.cod_enabled = 1, coalesce(c1.claim_value, pc.claim_money, v1.parcel_value), 0)) 超时效_COD妥投_预估金额
    ,sum(if(t1.result_rank = 3 and pi2.state = 5 and pi2.cod_enabled = 1, coalesce(c1.claim_value, pc.claim_money), 0)) 超时效_COD妥投_理赔金额
    -- 未达终态
    ,count(distinct if(t1.result_rank = 3 and pi2.state not in (5,7,8,9), t1.pno, null)) 超时效_未达终态_票数
    ,sum(if(t1.result_rank = 3 and pi2.state not in (5,7,8,9), coalesce(c1.claim_value, pc.claim_money, v1.parcel_value), 0)) 超时效_未达终态_预估金额
    ,sum(if(t1.result_rank = 3 and pi2.state not in (5,7,8,9), coalesce(c1.claim_value, pc.claim_money), 0)) 超时效_未达终态_理赔金额

    -- 丢失寄件人理赔
    ,count(distinct if(t1.result_rank = 2 and pct.pno is not null, t1.pno, null)) 丢失_寄件人理赔_票数
    ,sum(if(t1.result_rank = 2 and pct.pno is not null, coalesce(c1.claim_value, pc.claim_money, v1.parcel_value), 0)) 丢失_寄件人理赔_预估金额
    ,sum(if(t1.result_rank = 2 and pct.pno is not null, coalesce(c1.claim_value, pc.claim_money), 0)) 丢失_寄件人理赔_理赔金额
    -- 丢失网点理赔
    ,count(distinct if(t1.result_rank = 2 and pct.pno is null, t1.pno, null)) 丢失_网点理赔_票数
    ,sum(if(t1.result_rank = 2 and pct.pno is null, coalesce(c1.claim_value, pc.claim_money, v1.parcel_value), 0)) 丢失_网点理赔_预估金额
    ,sum(if(t1.result_rank = 2 and pct.pno is null, coalesce(c1.claim_value, pc.claim_money), 0)) 丢失_网点理赔_理赔金额
    -- 破损网点理赔
    ,count(distinct if(t1.result_rank = 1 and pct3.pno is null, t1.pno, null)) 破损_网点理赔_票数
    ,sum(if(t1.result_rank = 1 and pct3.pno is null, coalesce(c1.claim_value, pc.claim_money, v1.parcel_value), 0)) 破损_网点理赔_预估金额
    ,sum(if(t1.result_rank = 1 and pct3.pno is null, coalesce(c1.claim_value, pc.claim_money), 0)) 破损_网点理赔_理赔金额
    -- 破损收件人理赔
    ,count(distinct if(t1.result_rank = 1 and pct3.source in (4,6) and pct3.claim_target = 2 ,t1.pno, null)) 破损_收件人理赔_票数
    ,sum(if(t1.result_rank = 1 and pct3.source in (4,6) and pct3.claim_target = 2 ,coalesce(c1.claim_value, pc.claim_money, v1.parcel_value), 0)) 破损_收件人理赔_预估金额
    ,sum(if(t1.result_rank = 1 and pct3.source in (4,6) and pct3.claim_target = 2 ,coalesce(c1.claim_value, pc.claim_money), 0)) 破损_收件人理赔_理赔金额
    -- 破损寄件人理赔
    ,count(distinct if(t1.result_rank = 1 and pct3.source in (4,6) and pct3.claim_target = 1 ,t1.pno, null)) 破损_寄件人理赔_票数
    ,sum(if(t1.result_rank = 1 and pct3.source in (4,6) and pct3.claim_target = 1, coalesce(c1.claim_value, pc.claim_money, v1.parcel_value), 0)) 破损_寄件人理赔_预估金额
    ,sum(if(t1.result_rank = 1 and pct3.source in (4,6) and pct3.claim_target = 1, coalesce(c1.claim_value, pc.claim_money), 0)) 破损_寄件人理赔_理赔金额
    -- 破损仅外包装破损
    ,count(distinct if(t1.result_rank = 1 and pct3.source in (9,10), t1.pno, null)) 破损_仅外包装破损_票数
    ,sum(if(t1.result_rank = 1 and pct3.source in (9,10), coalesce(c1.claim_value, pc.claim_money, v1.parcel_value), 0)) 破损_仅外包装破损_预估金额
    ,sum(if(t1.result_rank = 1 and pct3.source in (9,10), coalesce(c1.claim_value, pc.claim_money), 0)) 破损_仅外包装破损_理赔金额
from t t1
left join val v1 on v1.pno = t1.pno
left join cla c1 on c1.pno = t1.pno
left join pcm pc on pc.pno = t1.pno
...
left join bi_pro.parcel_claim_task pct3 on pct3.pno = t1.pno and pct3.parcel_created_at > date_sub('${sdate}', interval 2 month) and pct3.source in (4,6,9,10) and pct.state < 7
left join
    (
        select
            pct.pno
