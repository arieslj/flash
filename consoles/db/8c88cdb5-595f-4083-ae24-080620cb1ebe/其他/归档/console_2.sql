...
select
    ...
    ,datediff(curdate(), plt.parcel_created_at) 揽收至今天数
    ...
    ,inv.date_cnt 近两月盘库天数
    ,case
        when bc.client_name = 'lazada' then la.whole_end_date
        when bc.client_name = 'shopee' then sp.oversla
        when bc.client_name = 'tiktok' then tt.end_7_date
    end 丢失时效
from bi_pro.parcel_lose_task plt
left join
    (
        select
            plt.pno
            ,count(distinct date (convert_tz(pr.routed_at, '+00:00', '+07:00'))) date_cnt
        from rot_pro.parcel_route pr
        join
            (
                select
...
left join dwm.dwd_ex_th_lazada_sla_detail la on la.pno = plt.pno
left join dwm.dwd_ex_th_shopee_pno_period sp on sp.pno = plt.pno
left join dwm.dwd_ex_th_tiktok_sla_detail tt on tt.pno = plt.pno
...
select
    ...
    ,dst.name   `目的地网点/สาขาปลายทาง`
    ,concat(pi.src_name, pi.src_phone, pi.src_detail_address) `发件人信息/ข้อมูลผู้ส่ง`
    ,concat(pi.dst_name, pi.dst_phone, pi.dst_detail_address) `收件人信息/ข้อมูลผู้รับ`
    ,pick.name `揽件网点/สาขารับงาน`
    ,pi.ticket_pickup_staff_info_id 揽件快递员
    ,convert_tz(pi.created_at, '+00:00', '+07:00') `揽收时间/เวลาที่รับพัสดุเข้าคลัง`
    ,convert_tz(fir.routed_at, '+00:00', '+07:00') `第一次打印面单时间เวลาพิมพ์เลเบลครั้งแรก`
    ,fir.store_name `第一次打印面单网点สาขาที่พิมพ์เลเบลครั้งแรก`
    ,fir.staff_info_id `第一次打印面单员工ID IDของพนักงานที่พิมพ์ฉลากเป็นครั้งแรก`
    ,convert_tz(fir2.routed_at, '+00:00', '+07:00') `第二次打印面单时间เวลาพิมพ์เลเบลครั้งที่สอง`
    ,fir2.store_name `第二次打印面单网点สาขาที่พิมพ์เลเบลครั้งที่สอง`
    ,fir2.staff_info_id `第二次打印员工的ID ID ของพนักงานที่พิมพ์ฉลากครั้งที่สอง`
    ,convert_tz(las.routed_at, '+00:00', '+07:00') `最后一次打印面单时间 เวลาพิมพ์เลเบลครั้งสุดท้าย`
    ,las.store_name `最后一次次打印面单网点 สาขาที่พิมพ์เลเบลครั้งสุดท้าย`
    ,las.staff_info_id `最后一次打印面单的员工ID ID ของพนักงานที่พิมพ์ฉลากครั้งล่าสุด`
    ,convert_tz(pi.finished_at, '+00:00', '+07:00') `包裹妥投时间/เวลาที่ยืนยันการส่งปลายทาง`
    ,fin.name `包裹妥投网点/สาขาที่ยืนยันการส่งปลายทาง`
    ,pi.ticket_delivery_staff_info_id '妥投员工IDปิดงานของพนักงาน ID'
    ,convert_tz(rp.routed_at, '+00:00', '+07:00') 第一次换单时间
    ,rp.store_name 第一次换单网点
    ,rp.staff_info_id 第一次换单员工ID
    ,convert_tz(rp2.routed_at, '+00:00', '+07:00') 最后一次换单时间
    ,rp2.store_name 最后一次换单网点
    ,rp2.staff_info_id 最后一次换单员工ID
from tmpale.tmp_th_pno_lj_0829 t
join fle_staging.parcel_info pi on pi.pno = t.pno
left join fle_staging.sys_store dst on dst.id = pi.dst_store_id
left join fle_staging.sys_store pick on pick.id = pi.ticket_pickup_store_id
left join fle_staging.sys_store fin on fin.id = pi.ticket_delivery_store_id
...
left join
    (
        ...
        from rot_pro.parcel_route pr
        join tmpale.tmp_th_pno_lj_0829 t1 on t1.pno = pr.pno
        where
            pr.routed_at > date_sub(curdate(), interval 2 month)
            and pr.route_action = 'PRINTING'
    ) fir on fir.pno = t.pno and fir.rk = 1
...
left join
    (
        ...
        where
            pr.routed_at > date_sub(curdate(), interval 2 month)
            and pr.route_action = 'PRINTING'
    ) fir2 on fir2.pno = t.pno and fir2.rk = 2
...
left join
    ...
    ) rp on rp.pno = t.pno and rp.rk = 1
left join
    (
        select
            pr.pno
        ...
        where
            pr.routed_at > date_sub(curdate(), interval 2 month)
            and pr.route_action = 'REPLACE_PNO'
    ) rp2 on rp2.pno = t.pno and rp2.rk = 1
