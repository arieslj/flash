    ...
    (
        ...
        (
            select
                ...
                ,pi.state pi_state
                ,hsi.job_title
                ,datediff(curdate(), hsi.hire_date) as hire_days
                ,pr.is_sub_staff
            ...
            from
                ...
                join bi_pro.hr_staff_info hsi on pr.father_staff_info_id=hsi.staff_info_id
...
from
    (
            ...
            #                   ,f1.负责人
                                ...
                                ,ifnull(concat(round(f6.非退件妥投量/f2.交接量_非退件*100,2),'%'), 0 ) as 妥投率
                                ...
                                ,case when f2.网点类型=1 and spill.滞留件积压预警='' and f6.非退件妥投量/f2.交接量_非退件<0.9 and ((ifnull(f6.非退件妥投量_大件折算,0)+ifnull(f6.退件妥投量_按地址转换,0))<90 ) then '是'
                                    when f2.网点类型=1 and spill.滞留件积压预警='Alert' and f6.非退件妥投量/f2.交接量_非退件<0.7 and ((ifnull(f6.非退件妥投量_大件折算,0)+ifnull(f6.退件妥投量_按地址转换,0))<90 ) then '是'
                                    when f2.网点类型=10 and spill.滞留件积压预警='' and f6.非退件妥投量/f2.交接量_非退件<0.9 and ((ifnull(f6.非退件妥投量_大件折算,0)+ifnull(f6.退件妥投量_按地址转换,0))<70 ) then '是'
                                    when f2.网点类型=10 and spill.滞留件积压预警 ='Alert' and f6.非退件妥投量/f2.交接量_非退件<0.7 and ((ifnull(f6.非退件妥投量_大件折算,0)+ifnull(f6.退件妥投量_按地址转换,0))<70) then '是'
                            ...
                            from
                                (
                                    ...
                                    left join dwm.dim_th_sys_store_rd dt on dt.store_id = swa.organization_id and dt.stat_date = date_sub(curdate(), interval 1 day)
                                    left join bi_pro.hr_job_title hjt on hjt.id = hsi.job_title
                            ...
                            left join
                                (# 快递员交接包裹后拨打电话情况
                                    select
                                        ...
                                        ,count(distinct case when  fn.before_17_calltimes is null and fn.pi_state not in(5,9) and  (fn.finished_at> date_add(curdate(), interval 10 hour) or fn.finished_at is null) then fn.pno else null end)/count(distinct fn.pno) as 交接包裹未拨打电话占比
                                    from  handover fn
                            ...
                            left join
                                ( -- 22点前最后一个妥投包裹时间
                                    select
                                        ...
                                        ,count(case when pi.returned=0 and hour(convert_tz(pi.finished_at,'+00:00','+07:00'))<22 then pi.pno else null end) as 非退件妥投量
