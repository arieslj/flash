...
select t.store_name
      ,t.staff_info_id
      ,sum(t.jiange) as 累计滞留天数
      ,round(avg(t.jiange),1) as 平均滞留天数
from (
  select
        pr1.store_name
       ,pr1.staff_info_id
       ,pr1.pno
       ,row_number()over(partition by pr1.pno order by pr1.routed_at desc) as rk1
       ,row_number()over(partition by pr2.pno order by pr2.routed_at) as rk2
       ,datediff(pr1.routed_at,pr2.routed_at) as jiange
  from
      rot_pro.parcel_route pr1
  left join
      rot_pro.parcel_route pr2 on pr1.pno = pr2.pno and pr1.store_id = pr2.store_id
  where pr1.route_action = 'DELIVERY_TICKET_CREATION_SCAN'
    and pr2.route_action = 'ARRIVAL_WAREHOUSE_SCAN'
    and pr1.routed_at >= date_sub(curdate(), interval 7 hour)
    and pr1.store_name like '%SP%'
    -- and pr1.staff_info_id = '706440'
    -- and pr1.pno = 'THT04021UB8S85Z'
)t
