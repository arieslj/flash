...
from
    (
        ...
        from
            ...
            (
                select
                    count(if(timestampdiff(minute , ear.task_cancel_time, ear.updated_at)/60 > 72, ear.id, null)) delay_cnt
                     ,count(ear.id) total_cnt
                     ,count(if(timestampdiff(minute , ear.task_cancel_time, ear.updated_at)/60 <= 72, ear.id, null)) / count(ear.id) rate
                from wrs_production.epop_audit_record ear
                where
                    ear.updated_at > date_sub('${sdate}', interval 7 hour)
                  and ear.updated_at < date_add(date_sub('${edate}', interval 7 hour), interval 1 day)
