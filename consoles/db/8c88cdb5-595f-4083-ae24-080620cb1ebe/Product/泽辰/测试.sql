...
from
    (
        ...
        left join bi_pro.staff_achievements_give_object_day sag on sag.staff_info_id = am.staff_info_id and sag.stat_date = date_sub(curdate(), interval 1 day)
        left join bi_pro.hr_job_title hjt on hjt.id = sag.job_title
        ...
        where
            ...
            and (hsi.hire_type = 13 or sag.hire_type = 13)
            and am.state = 1
            and am.isdel = 0
...
from
    (
        ...
        left join dwm.dwd_dim_dict ddd on ddd.element = am.route_action and ddd.db = 'rot_pro' and ddd.tablename = 'parcel_route' and ddd.fieldname = 'route_action'
        where
            am.created_at >= date_sub(last_day(date_sub(curdate(), interval 1 month)), interval day(last_day(date_sub(curdate(), interval 1 month)))  -1 day)
            and am.created_at < date_sub(last_day(date_sub(curdate(), interval 1 month)), interval day(last_day(date_sub(curdate(), interval 1 month))) - 16 day)
            -- and hsi.job_title in (13,110,452,1497)
            and (hsi.hire_type = 13 or sag.hire_type = 13)
            and am.state = 1
            and am.isdel = 0
