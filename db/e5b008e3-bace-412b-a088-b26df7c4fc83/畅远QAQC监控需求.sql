...
from
    (
        ...
        select
            ...
            ,a.闪速认定任务状态
            ,a.created_at 创建时间
            ,if(a.闪速认定任务状态 in ('包裹未丢失', '丢失件处理完成'), a.updated_at, null ) 处理时间
            ,a.operator_id 操作人
            ,case
                when a.闪速认定任务状态 in ('包裹未丢失', '丢失件处理完成') and a.updated_at <= a.sla_date and a.operator_id in (10000,10001) then '时效内自动处理'
                when a.闪速认定任务状态 in ('包裹未丢失', '丢失件处理完成') and a.updated_at <= a.sla_date and a.operator_id not in (10000,10001) then '时效内人工处理'
                when a.闪速认定任务状态 in ('包裹未丢失', '丢失件处理完成') and a.updated_at > a.sla_date and a.operator_id in (10000,10001) then '时效外自动处理'
                when a.闪速认定任务状态 in ('包裹未丢失', '丢失件处理完成') and a.updated_at > a.sla_date and a.operator_id not in (10000,10001) then '时效外人工处理'
                else '未处理'
            end 时效判断
        from
            (
                select
                    ...
                    ,if(plt.source in (3,12), 'C、L来源', '其他来源') CL来源判断
                    ,case plt.state
                        when 1 then '丢失件待处理'
