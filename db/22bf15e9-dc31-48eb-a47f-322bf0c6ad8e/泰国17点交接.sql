                ...
                from
                    (
                        select
                            ...
                            ,dp.region_name
                            ,pi.state pi_state
                            ,convert_tz(pr.routed_at,'+00:00','+07:00') as routed_at
                            ,convert_tz(pi.finished_at,'+00:00','+07:00') as finished_at
                            ,date(convert_tz(pr.routed_at,'+00:00','+07:00'))  as routed_date
                            ,row_number() over(partition by pr.pno,date(convert_tz(pr.routed_at,'+00:00','+07:00')) order by convert_tz(pr.routed_at,'+00:00','+07:00') desc) as rnk
                        from rot_pro.parcel_route pr
                        left join dwm.dim_th_sys_store_rd dp on dp.store_id = pr.store_id and dp.stat_date = date_sub(curdate(), interval 1 day)
                        left join fle_staging.parcel_info pi on pr.pno = pi.pno
                        where
                            pr.routed_at >= date_sub(curdate(), interval 7 hour)
                            and pr.routed_at < date_add(curdate(), interval 17 hour)
                            and pr.route_action in ('DELIVERY_TICKET_CREATION_SCAN')
                            and dp.store_category in (1,10)  -- 限制SP,BDC
                    ) pr
                where
                    pr.rnk=1
            ) pr
