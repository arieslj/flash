...
(
    SELECT
        hdw.stat_date
        ,hdw.device_id
        ,sum(hdw.weight_offset_parcel_count) ct
        ,hdw.weight_parcel_count
    from hbi.hub_device_weight hdw
    where hdw.weight_offset_parcel_count>=-200
    and hdw.weight_offset_parcel_count<=200
    and hdw.stat_date>=date_sub(CURRENT_DATE(),interval 62 day)
    group by 1,2
),
sor AS
(
	select
		...
		,dsa.created_at
		,dsa.staff_info_id
		,row_number()over(partition by dsa.oss_bucket_key order by dsa.created_at) rn
	from dwm.dwd_sorting_attachment dsa
	where dsa.oss_bucket_type in ('SORT_PARCEL_INFO')
	and dsa.p_date >=date_sub(CURRENT_DATE(),interval 62 day)
),
fc AS
(
	select
    	pwr.pno
        ,ss2.name
        ,pwr.after_weight/1000  weight
        ,CONCAT_WS('*',pwr.after_length,pwr.after_width,pwr.after_height) lwh
        ,pwr.staff_info_id
        ,pwr.device_id
        ,pwr.created_at
        ,ROW_NUMBER() over(PARTITION by pwr.pno order by pwr.created_at) rn
    from dwm.drds_parcel_weight_revise_record_d pwr
    left join bi_pro.sys_store ss2 on ss2.id = pwr.store_id
    where pwr.event=3
)
...
SELECT
    ...
    ,ss.name 揽件网点
    ,pi.ticket_pickup_staff_info_id 揽件员工ID
    ,pi.exhibition_weight /1000 揽收重量
    ,CONCAT_WS('*',pi.exhibition_length ,pi.exhibition_width ,pi.exhibition_height ) 揽收尺寸
    ,dsa.url 揽收照片
    ,pwr.weight 称重重量
    ,pwr.lwh 称重尺寸
    ,dsa.device_id 称重设备ID
    ,h0.ct/h0.weight_parcel_count 称重准确重量占比
    ,dsa.staff_info_id 操作员工ID
    ,f1.name 第一次分拣复称网点
    ,f1.weight 第一次分拣复称重量
    ,f1.lwh 第一次分拣复称尺寸
    ,s1.url 第一次分拣复称照片
    ,f1.device_id 第一次分拣复称设备ID
    ,h1.ct/h1.weight_parcel_count 第一次分拣复称设备称重准确重量占比
    ,f1.staff_info_id 第一次分拣复称操作员工ID
    ,f2.name 第二次分拣复称网点
    ,f2.weight 第二次分拣复称重量
    ,f2.lwh 第二次分拣复称尺寸
    ,s2.url 第二次分拣复称照片
    ,f2.device_id 第二次分拣复称设备ID
    ,h2.ct/h2.weight_parcel_count 第二次分拣复称设备称重准确重量占比
    ,f2.staff_info_id 第二次分拣复称操作员工ID
from
(
    select
        pi.pno
    ...
    where pi.created_at>=date_sub(CURRENT_DATE(),interval 62 day)
)pi
left JOIN
(
    select
	...
	where oi.created_at>=date_sub(CURRENT_DATE(),interval 90 day)
)oi on pi.pno=oi.pno
left JOIN
(
	select
...
)ss on pi.ticket_pickup_store_id=ss.id
left JOIN
(-- 揽收照片
	select
		dsa.oss_bucket_key
		...
		,row_number()over(partition by dsa.oss_bucket_key order by dsa.created_at) rn
	from dwm.dwd_sorting_attachment dsa
	where dsa.oss_bucket_type in ('DWS_PARCEL_WEIGHT_INFO','PARCEL_UPDATE_WEIGHT_INFO')
	and dsa.created_at>=date_sub(CURRENT_DATE(),interval 91 day)
)dsa on dsa.oss_bucket_key=pi.pno and dsa.rn=1
left join
(
	select
		pwr.pno
        ...
        ,CONCAT_WS('*',pwr.after_length,pwr.after_width,pwr.after_height) lwh
	from dwm.drds_parcel_weight_revise_record_d pwr
)pwr on pwr.pno=dsa.oss_bucket_key and pwr.device_id=dsa.device_id
left join hbi h0 on h0.device_id=dsa.device_id and h0.stat_date =dsa.p_date
-- 分拣机复称图片
left JOIN sor s1 on s1.oss_bucket_key=pi.pno and s1.rn=1
left JOIN sor s2 on s2.oss_bucket_key=pi.pno and s2.rn=2
-- 分拣机复称信息
left join fc f1 on f1.pno=pi.pno and f1.rn=1
left join hbi h1 on h1.device_id=f1.device_id and h1.stat_date =date(date_add(f1.created_at,interval 7 hour))
left join fc f2 on f2.pno=pi.pno and f2.rn=2
left join hbi h2 on h2.device_id=f2.device_id and h2.stat_date =date(date_add(f2.created_at,interval 7 hour))
-- where pi.pno='TH01474CNV5V7B'
where pi.`pno` in('${SUBSTITUTE(SUBSTITUTE(p2,"\n",","),",","','")}')
limit 5000;
select * from dwm.dwd_sorting_attachment dsa
select * from fle_staging.parcel_info pi limit 10