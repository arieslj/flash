                    ...
                    (
                        select
                            ...
                            ,case
                                ...
                                when bc.client_id is null and kp.id is not null  and kp.department_id = 5103 then 'KAM'
                                when bc.client_id is null and kp.id is not null  and kp.department_id != 5103 then 'KA'
                                else 'GE'
                            end client_name
                            ,case
                        ...
                        from
                            (
                                ...
                                left join ph_staging.parcel_info pi on pi.pno = plt.pno and pi.created_at > date_sub('${sdate}', interval 2 month)
                                where
                                    plt.parcel_created_at >= '${sdate}' and plt.parcel_created_at < date_add('${edate}', interval 1 day) and plt.state = 6 and plt.duty_result in (1,2)
                                ...
                                from ph_bi.parcel_lose_task plt
                                left join ph_staging.parcel_info pi on pi.pno = plt.pno and pi.created_at > date_sub('${sdate}', interval 2 month)
                                left join ph_bi.parcel_lose_task plt2 on plt2.pno = pi.customary_pno and plt2.source = 11 and plt2.parcel_created_at > date_sub('${sdate}', interval 2 month)
                                where
                                    plt.source = 11 and plt.parcel_created_at >= '${sdate}'
                                    and plt.parcel_created_at < date_add('${edate}', interval 1 day)
...
#             ,pi2.insure_declare_value/100
            ,case
                when t1.client_name = 'lazada' then oi.insure_declare_value/100
                when t1.client_name = 'shopee' and coalesce(if(oi.cogs_amount = 0, null, oi.cogs_amount)/100, p2.cod_amount/100) >= 4000 then 4000
                when t1.client_name = 'shopee' and coalesce(if(oi.cogs_amount = 0, null, oi.cogs_amount)/100, p2.cod_amount/100) < 4000 then coalesce(if(oi.cogs_amount = 0, null, oi.cogs_amount)/100, p2.cod_amount/100)
                when t1.client_name = 'tiktok' and coalesce(if(oi.cogs_amount = 0, null, oi.cogs_amount)/100, p2.cod_amount/100) >= 3000 then 3000
                when t1.client_name = 'tiktok' and coalesce(if(oi.cogs_amount = 0, null, oi.cogs_amount)/100, p2.cod_amount/100) < 3000 then coalesce(if(oi.cogs_amount = 0, null, oi.cogs_amount)/100, p2.cod_amount/100)
                when t1.client_name = 'shein' and coalesce(if(oi.cogs_amount = 0, null, oi.cogs_amount)/100, p2.cod_amount/100) >= 3000 then 3000
                when t1.client_name = 'shein' and coalesce(if(oi.cogs_amount = 0, null, oi.cogs_amount)/100, p2.cod_amount/100) < 3000 then coalesce(if(oi.cogs_amount = 0, null, oi.cogs_amount)/100, p2.cod_amount/100)
                when t1.client_name in ('KA','GE') and coalesce(if(oi.cogs_amount = 0, null, oi.cogs_amount)/100, p2.cod_amount/100) >= 3000 then 3000
                when t1.client_name in ('KA','GE') and coalesce(if(oi.cogs_amount = 0, null, oi.cogs_amount)/100, p2.cod_amount/100) < 3000 then coalesce(if(oi.cogs_amount = 0, null, oi.cogs_amount)/100, p2.cod_amount/100)
                else 0
            end parcel_value
            ,p2.store_total_amount/100 store_total_amount
        from t t1
        left join ph_staging.parcel_info p1 on t1.pno = p1.pno
        left join ph_staging.parcel_info p2 on p2.pno = if(p1.returned = 1, p1.customary_pno, p1.pno)
        left join ph_staging.order_info oi on oi.pno = if(p1.pno is null, t1.pno, p2.pno)
    )
, cla as
    (
        select
...
, pcm as
    (
        ...
        from ph_staging.pickup_claims_ticket pct
        join t t1 on t1.pno = pct.pno
        where
...
select
    ...
    ,count(distinct t1.pno)  总体_数量
    ,sum(v1.store_total_amount) 总体_运费
    ,sum(coalesce(c1.claim_value, pc.claim_money, v1.parcel_value)) 总体_预计涉及金额
    ,sum(coalesce(c1.claim_value, pc.claim_money)) 总体_实际赔付金额

    ,count(distinct if(t1.result_rank = 3, t1.pno, null))  超时效_丢失数量
    ,sum(if(t1.result_rank = 3, v1.store_total_amount, null)) 超时效_运费
    ,sum(if(t1.result_rank = 3, coalesce(c1.claim_value, pc.claim_money, v1.parcel_value), null)) 超时效_超时效预计涉及金额
    ,sum(if(t1.result_rank = 3, coalesce(c1.claim_value, pc.claim_money), null)) 超时效_实际赔付金额

    ,count(distinct if(t1.result_rank = 2, t1.pno, null))  遗失_丢失数量
    ,sum(if(t1.result_rank = 2, v1.store_total_amount, null)) 遗失_运费
    ,sum(if(t1.result_rank = 2, coalesce(c1.claim_value, pc.claim_money, v1.parcel_value), null)) 遗失_超时效预计涉及金额
    ,sum(if(t1.result_rank = 2, coalesce(c1.claim_value, pc.claim_money), null)) 遗失_实际赔付金额

    ,count(distinct if(t1.result_rank = 1, t1.pno, null))  破损_丢失数量
    ,sum(if(t1.result_rank = 1, v1.store_total_amount, null)) 破损_运费
    ,sum(if(t1.result_rank = 1, coalesce(c1.claim_value, pc.claim_money, v1.parcel_value), null)) 破损_超时效预计涉及金额
    ,sum(if(t1.result_rank = 1, coalesce(c1.claim_value, pc.claim_money), null)) 破损_实际赔付金额
from t t1
...
select
    t1.client_name
    ,count(distinct if(pi.state = 8 and pi.dst_store_id = 'PH19040F05' and t1.result_rank = 3, t1.pno, null)) 拍卖仓妥投成功_超时效 -- PN5-CS(INVENTORY)
    ,sum(if(pi.state = 8 and pi.dst_store_id = 'PH19040F05' and t1.result_rank = 3, v1.parcel_value, null)) 拍卖仓妥投成功涉及金额_超时效
    ,count(distinct if(pi.state = 8 and pi.dst_store_id != 'PH19040F05' and t1.result_rank = 3, t1.pno, null)) 丢失_超时效 -- PN5-CS(INVENTORY)
    ,sum(if(pi.state = 8 and pi.dst_store_id != 'PH19040F05' and t1.result_rank = 3, v1.parcel_value, null)) 丢失涉及金额_超时效
    ,count(distinct if(pi.state not in (5,7,8,9) and t1.result_rank = 3, t1.pno, null)) 未终态_超时效 -- PN5-CS(INVENTORY)
    ,sum(if(pi.state not in (5,7,8,9) and t1.result_rank = 3, v1.parcel_value, null)) 未终态涉及金额_超时效
    ,count(distinct if(pi.state = 5 and t1.result_rank = 3, t1.pno, null)) 妥投_超时效
    ,sum(if(pi.state = 5 and t1.result_rank = 3, v1.parcel_value, null)) 妥投涉及金额_超时效
...
from t t1
left join val v1 on v1.pno = t1.pno
left join ph_staging.parcel_info pi on pi.pno = t1.pno and pi.created_at >= date_sub('${sdate}', interval 3 month)
left join ph_staging.parcel_route pr on pr.pno = t1.pno and pr.routed_at > date_sub('${sdate}', interval 3 month) and pr.route_action = 'DELIVERY_CONFIRM'
group by 1
...
, val as
    (
        ...
        left join ph_staging.order_info  pai on pai.pno = if(pi.pno is null, t1.pno, pi2.pno)
...
select
    ...
    ,duty.duty_store 责任网点
    ,c1.finance_updated_at 支付理赔时间
    ,case c1.duty_result
        when 1 then '丢失'
        when 2 then '破损'
        when 3 then '超时效'
...
left join val v1 on v1.pno = t1.pno
left join cla c1 on c1.pno = t1.pno
left join ph_staging.parcel_info pi on pi.pno = t1.pno and pi.created_at >= date_sub('${sdate}', interval 2 month )
left join ph_staging.parcel_route pr on pr.pno = pi.pno and pr.routed_at > date_sub('${sdate}', interval 8 hour) and pr.route_action = 'DELIVERY_CONFIRM'
left join
    (
        select
            t1.id
...
left join
    (
        ...
        left join ph_bi.parcel_claim_negotiation pcn on pcn.task_id = pct.id
        where
            pct.created_at > date_sub('${sdate}', interval 2 month)
