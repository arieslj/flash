select
    ...
    ,convert_tz(di.created_at, '+00:00', '+07:00')  '问题件生成/转交时间'
    ,convert_tz(di.updated_at, '+00:00', '+07:00')  协商结果处理时间
    ...
    ,'' 客户处理状态
    ,'拒收策略-由商户决定' 类型
from fle_staging.ka_diff_ticket kdt
-- left join fle_staging.customer_diff_ticket cdt on cdt.diff_info_id = kdt.diff_info_id
left join fle_staging.diff_info di on di.id = kdt.diff_info_id
...
select
    ...
    ,cdt.client_id 客户ID
    ,ddd.CN_element 问题件类型
    ,convert_tz(cdt.user_operated_start_at, '+00:00', '+07:00')  '问题件生成/转交时间'
    ,convert_tz(cdt.updated_at, '+00:00', '+07:00')  协商结果处理时间
...
left join dwm.dwd_dim_dict ddd on ddd.element = di.diff_marker_category and ddd.db = 'fle_staging' and ddd.tablename = 'diff_info' and ddd.fieldname = 'diff_marker_category'
where
    cdt.created_at > '2024-04-30 17:00:00'
    and cdt.created_at < '2024-05-31 17:00:00'
...
select
    ...
    ,cdt.client_id 客户ID
    ,ddd.CN_element 问题件类型
    ,convert_tz(cdt.created_at, '+00:00', '+07:00')  '问题件生成/转交时间'
    ,convert_tz(cdt.updated_at, '+00:00', '+07:00')  协商结果处理时间
    ,case cdt.negotiation_result_category # 协商结果
        when 1 then '赔偿' -- 丢弃并赔偿（关闭订单，网点自行处理包裹）
        when 2 then '关闭订单(不赔偿不退货)' -- 丢弃（关闭订单，网点自行处理包裹）
