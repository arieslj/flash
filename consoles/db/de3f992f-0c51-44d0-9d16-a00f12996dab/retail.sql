select
    ...
    ,case
        ...
        when a.`channel_type` =11 then '其他'
        when a.`channel_type` =13 then '疑似违规回访'
        when a.`channel_type` =14 then 'APP异常包裹投诉'
        when a.`channel_type` =15 then '揽收调度平台'
        when a.`channel_type` =16 then '询问任务'
    end as '投诉渠道'
