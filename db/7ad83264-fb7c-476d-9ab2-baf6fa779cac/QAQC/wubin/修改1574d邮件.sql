  /*=====================================================================+
    表名称： 1574d_ph_KAM_problem_feedback
    功能描述： KAM问题件反馈
...
left join
    (
        select
            scr.pno
            ,scr.reason
        from ph_drds.shopee_callback_record scr
        where
            scr.created_at >= date_sub(curdate(),interval 1 week)
            and scr.status_code = 120
    )  scr on scr.pno = pr.pno
where
    1=1
