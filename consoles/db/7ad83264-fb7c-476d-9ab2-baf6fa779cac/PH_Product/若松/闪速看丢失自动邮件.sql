-- 未判责A来源
    ...
    (
         select
            ...
            ,pi.dst_name 收件人Recipient
            ,pi.dst_phone 收件人电话RecipientPhone
        ...
        where
            ...
            and plt.created_at > date_sub(curdate(), interval 3 month)
    )
select
   t1.*
...
with t as
    (
        ...
        left join ph_staging.sys_store ss on ss.id = plt.last_valid_store_id
        where
            plt.source in (1,3,33)
            and plt.state = 6
            and plt.updated_at >= date_sub(curdate(), interval 1 month )
            and plt.penalties > 0
    )
select
   t1.*
