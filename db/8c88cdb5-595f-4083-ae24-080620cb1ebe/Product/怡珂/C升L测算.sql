...
from
    (
        ...
        join fle_staging.sys_store ss on ss.id = pi.duty_store_id
        join rot_pro.parcel_route pr on pr.pno = pi.pno
        where
            pi.created_at > '2024-09-30 17:00:00'
...
where
    pr.rk = 1
    and timestampdiff(hour, pr.routed_at, now()) > 55 -- 48 + 7
...
from
    (
        ...
        from fle_staging.parcel_info pi
        join fle_staging.sys_store ss on ss.id = pi.duty_store_id
        join rot_pro.parcel_route pr on pr.pno = pi.pno and pr.route_action = 'RECEIVED'
        where
            pi.created_at > '2024-09-30 17:00:00'
            and pi.state = 1
            and ss.category not in (8,12) -- 非HUB
    ) pr
left join nl_production.abnormal_white_list awl on awl.store_id = pr.id and awl.type = 2 and awl.start_date <= date_sub(curdate(), 1) and awl.end_date >= date_sub(curdate(), 1)
where
    pr.rk = 1
    and timestampdiff(hour , pr.routed_at, now()) > 31 -- 24 + 7
# group by 1
...
where
    ...
    and timestampdiff(hour , pr.routed_at, now()) > 31
# group by 1
...
where
    ...
    and timestampdiff(hour , pr.routed_at, now()) > 175
# group by 1
...
from
    (
        select
            ...
            ,convert_tz(coalesce(ft.sign_time, ft.real_arrive_time, ft.plan_arrive_time), '+07:00', '+00:00') use_time_at
        from
            (
                select
                    pr.pno
                    ...
                    (
                        ...
                        left join
                            (
                                ...
                                where
                                    pr.routed_at > '2024-09-30 17:00:00'
                                    and pr.route_action in ('RECEIVED','STAFF_INFO_UPDATE_WEIGHT','RECEIVE_WAREHOUSE_SCAN','STORE_KEEPER_UPDATE_WEIGHT','STORE_SORTER_UPDATE_WEIGHT','DELIVERY_TICKET_CREATION_SCAN','ARRIVAL_WAREHOUSE_SCAN','ARRIVAL_WAREHOUSE_SCAN','SHIPMENT_WAREHOUSE_SCAN','DETAIN_WAREHOUSE','DELIVERY_CONFIRM','DIFFICULTY_HANDOVER','DELIVERY_MARKER','REPLACE_PNO','PRINTING','SEAL','UNSEAL','PARCEL_HEADLESS_PRINTED','THIRD_EXPRESS_ROUTE','PICKUP_RETURN_RECEIPT','FLASH_HOME_SCAN','SORTING_SCAN','CANCEL_SHIPMENT_WAREHOUSE','INVENTORY','REFUND_CONFIRM','DISTRIBUTION_INVENTORY','DELIVERY_TRANSFER','DELIVERY_PICKUP_STORE_SCAN','ACCEPT_PARCEL','DIFFICULTY_HANDOVER')
                ...
                left join rot_pro.parcel_route pr2  on pr2.pno = pr.pno and pr2.route_action = 'SHIPMENT_WAREHOUSE_SCAN' and pr2.next_store_id = pr.id
            ) a1
        left join bi_pro.fleet_time ft on ft.proof_id = json_extract(a1.extra_value, '$.proofId') and ft.next_store_id = a1.id
        where
            a1.rk = 1
    )  a2
left join nl_production.abnormal_white_list awl on awl.store_id = a2.id and awl.type = 2 and awl.start_date <= date_sub(curdate(), 1) and awl.end_date >= date_sub(curdate(), 1)
left join
    (
        select
            pr.pno
            ,pr.routed_at
        from rot_pro.parcel_route pr
        join fle_staging.parcel_info pi2 on pi2.pno = pr.pno
        where
            pr.routed_at > '2024-09-30 17:00:00'
            and pi2.created_at > '2024-09-30 17:00:00'
            and pi2.state not in (1,3,5,7,8,9)
            and pr.route_action in ('RECEIVED','STAFF_INFO_UPDATE_WEIGHT','RECEIVE_WAREHOUSE_SCAN','STORE_KEEPER_UPDATE_WEIGHT','STORE_SORTER_UPDATE_WEIGHT','DELIVERY_TICKET_CREATION_SCAN','ARRIVAL_WAREHOUSE_SCAN','ARRIVAL_WAREHOUSE_SCAN','SHIPMENT_WAREHOUSE_SCAN','DETAIN_WAREHOUSE','DELIVERY_CONFIRM','DIFFICULTY_HANDOVER','DELIVERY_MARKER','REPLACE_PNO','PRINTING','SEAL','UNSEAL','PARCEL_HEADLESS_PRINTED','THIRD_EXPRESS_ROUTE','PICKUP_RETURN_RECEIPT','FLASH_HOME_SCAN','SORTING_SCAN','CANCEL_SHIPMENT_WAREHOUSE','INVENTORY','REFUND_CONFIRM','DISTRIBUTION_INVENTORY','DELIVERY_TRANSFER','DELIVERY_PICKUP_STORE_SCAN','ACCEPT_PARCEL','DIFFICULTY_HANDOVER')
    ) pr on pr.pno = a2.pno and pr.routed_at > a2.use_time_at
where
    pr.pno is null
    and timestampdiff(hour, a2.use_time_at, now()) > 55 -- 24+7
# group by 1