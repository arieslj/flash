...
(
  ...
  (
      ...
      (
        ...
        from
        (
         select distinct pr.pno,date(convert_tz(pr.routed_at,'+00:00', '+08:00')) routed_at
  ...
  and pi.created_at<=date_add(convert_tz('${edate}','+08:00','+00:00'),interval 1 day)
           and pi.state not in (5,7,8,9)
         )t on pr.pno=t.pno and pr.store_id=t.dst_store_id
  ...
  and pi.created_at<=date_add(convert_tz('${edate}','+08:00','+00:00'),interval 1 day)
         ...
         where pr.route_action<>'INVENTORY'
         and pr.routed_at>=convert_tz('${sdate}','+08:00','+00:00')
        )prin on prin.pno=pr.pno and prin.routed_at=pr.routed_at
        where prin.pno is null
      )a
  )b
  group by 1,2
...
select
    ...
    ,case
        when t1.state=6 and cs.client_id is not null and (di.diff_marker_category not in (32,69,7,22) or di.diff_marker_category is null) then '总部CS'
        when t1.state=6 and lts.pno is not null then '总部CS'
        when t1.state=6 and lts2.pno is not null then '调度'
        when t1.state=6 and bc.client_name is null and kp.department_id='388' and (di.diff_marker_category not in (32,69,7,22) or di.diff_marker_category is null) then 'KAM'
        when t1.state=6 and bc.client_name='lazada' and (di.diff_marker_category not in (32,69,7,22) or di.diff_marker_category is null) then 'PMD_LAZ'
        when t1.state=6 and bc.client_name='tiktok' and (di.diff_marker_category not in (32,69,7,22) or di.diff_marker_category is null) then 'PMD_TT'
        when t1.state=6 and di.diff_marker_category in (32,69,7,22) then '调度'
        when t1.state=6 then 'SP'
        when t1.state<>6 and pr.store_id = t1.ticket_pickup_store_id and plt2.pno is null and (b.cnt<3 or b.cnt is null)
        ...
        when t1.state<>6 and pr.store_name in ('IT&Products','Customer Service') and pr1.category in (1,4,11) then 'SP'
        when t1.state<>6 and pr.store_name in ('IT&Products','Customer Service') and pr1.store_name like 'FH%' then 'FH'
        when t1.state<>6 and pr.store_name in ('IT&Products','Customer Service') and pr1.store_name like '%HUB%' then 'HUB'
        when t1.state<>6 and pr.store_name in ('IT&Products','Customer Service') and pr1.store_name like 'BDC%' then 'BDC'
    ...
    ,pr_time.trytimes 尝试派件次数
    ,case when bc.client_name='lazada' then lp.whole_end_date
...
left join
   ...
   )cs on t1.client_id=cs.client_id
left join
(
select a.*
from
(
  ...
  where pr.pno_created_at>=convert_tz('${sdate}','+08:00','+00:00')
 ) a
 where rk=1
) pr on pr.pno = t1.pno
left join
...
(
  ...
  and di.created_at>=convert_tz('${sdate}','+08:00','+00:00')
) di on di.pno = t1.pno
...
(
  select
    pr.pno
   ,count(distinct pr.routed_date) trytimes
  from
  (
    select
        pr.pno
    ...
    from  my_staging.parcel_route pr
    join t t3 on t3.pno = pr.pno
    where pr.route_action in ('DELIVERY_TICKET_CREATION_SCAN','DELIVERY_MARKER')
    and pr.routed_at>=convert_tz('${sdate}','+08:00','+00:00')
   )pr
   where pr.rk=1
   group by 1
...
(
  select
   ...
   and lt.state =6
   and lt.created_at>='${sdate}'
)lt_duty on t1.pno=lt_duty.pno
left join
(
select
    *
  ...
  (
    ...
    where di.created_at>=convert_tz('${sdate}','+08:00','+00:00')
   )di
   where di.rk=1
)di_des on t1.pno=di_des.pno
