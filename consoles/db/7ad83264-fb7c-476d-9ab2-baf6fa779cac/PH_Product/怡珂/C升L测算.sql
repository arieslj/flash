...
from
    (
        ...
        join ph_staging.sys_store ss on ss.id = pi.duty_store_id
        join ph_staging.parcel_route pr on pr.pno = pi.pno and pr.store_id = pi.duty_store_id
        where
...
from
    (
        ...
        from ph_staging.parcel_info pi
        join ph_staging.sys_store ss on ss.id = pi.duty_store_id
        join ph_staging.parcel_route pr on pr.pno = pi.pno and pr.route_action = 'RECEIVED'
        where
            pi.created_at > '2024-09-30 16:00:00'
            and pi.state = 1
...
from
    (
        ...
        from ph_staging.parcel_info pi
        join ph_staging.sys_store ss on ss.id = pi.duty_store_id
        join ph_staging.parcel_route pr on pr.pno = pi.pno and pr.route_action = 'DELIVERY_TICKET_CREATION_SCAN'
        where
            pi.created_at > '2024-09-30 16:00:00'
            and pi.state = 3
            and ss.category not in (8,12) -- 非HUB
    ...
    (
        ...
        from
            (
                ...
                left join ph_staging.parcel_route pr2  on pr2.pno = pr.pno and pr2.route_action = 'SHIPMENT_WAREHOUSE_SCAN' and pr2.next_store_id = pr.id
            ) a1
        left join ph_bi.fleet_time ft on ft.proof_id = json_extract(a1.extra_value, '$.proofId') and ft.next_store_id = a1.id
        where
            a1.rk = 1
    )  a2
...
left join
    (
        select
            ...
            ,pr.routed_at
        from ph_staging.parcel_route pr
        join ph_staging.parcel_info pi2 on pi2.pno = pr.pno
        where
            pr.routed_at > '2024-09-30 16:00:00'
