select
    ...
    ,count(distinct if(a.回复内容 in ('Y', 'y'),a.task_id,null)) 回访结果属实量
    ,count(distinct if(a.回复内容 in ('Y', 'y'),a.task_id,null))/count(distinct if(a.回复时间 is not null,a.task_id,null)) '回访结果属实率（回访结果不属实量/已回复量）'
    ,count(distinct if(a.回复内容 in ('N', 'n'),a.task_id,null)) 回访结果不属实量
    ,count(distinct if(a.回复内容 in ('N', 'n'),a.task_id,null))/count(distinct if(a.回复时间 is not null,a.task_id,null)) '虚假率（回访结果不属实量/已回复量）'
    ,count(distinct if(a.回复内容 not in ('Y', 'N', 'y', 'n'),a.task_id,null)) 无效回复量
    ,count(distinct if(a.回复内容 not in ('Y', 'N', 'y', 'n'),a.task_id,null))/count(distinct if(a.回复时间 is not null,a.task_id,null)) 无效回复率
...
left join nl_production.viber_return_visit vr on om.stat_date=vr.stat_date and om.phone=vr.mobile
where
    om.src='bi_viber_visit'
    and om.service_provider=9
    and om.send_at>='${sdate}'
    and om.send_at<date_add('${edate}',interval 1 day)
    and REPLACE (json_extract(json_extract(json_extract(im.content, '$.payload'),'$.content'),'$.text'),'"','')  not in ('Y','N')