...
left join my_staging.fleet_van_proof fvp on fvp.id = substring_index(plt.fleet_routeids, '/', 1)
left join my_staging.sys_store ss3 on ss3.id = plt.last_valid_store_id
left join my_bi.work_order wo on wo.loseparcel_task_id = plt.id
where
    plt.state in (5,6)
    and plt.updated_at >= '2024-01-01'
    and plt.updated_at < '2024-02-01'
    and plt.operator_id not in (10000,10001)
    and if(pi.state = 5, 1 = 1, plt.penalties > 0)
group by plt.id