    ...
    (
        select
          ...
          --  ,if(pci.created_at < date_add(date(pci.created_at), interval 16 hour), date_add(date(pci.created_at), interval 24 hour), date_add(date(pci.created_at), interval 36 hour))
            ,case
                when pci.client_type in (1,2,3,4) then date_add(date(pci.created_at), interval 24 hour)
                else date_add(pci.created_at, interval 48 hour)
            end short_sla
            ,date_add(pci.created_at, interval 48 hour) long_sla
        from my_bi.parcel_complaint_inquiry pci
        where
            pci.created_at >= '${start_date}'
            and pci.created_at < date_add('${end_date}', interval 1 day)
...
left join tmpale.tmp_my_client_type_lj t on t.client_id = acc.client_id
where
...
select
    ...
    ,count(acc.id) 整体
from my_bi.abnormal_customer_complaint acc
left join my_bi.parcel_complaint_inquiry pci on acc.pno = pci.merge_column
left join tmpale.tmp_my_client_type_lj t on t.client_id = acc.client_id
where
    acc.channel_type = 16 -- 询问任务
    and acc.created_at >= '${start_date}'
...
select
    ...
    ,count(pci.id) 整体
from my_bi.parcel_complaint_inquiry pci
join my_bi.abnormal_message am on json_extract(am.extra_info, '$.id') = pci.id
where
    pci.created_at >= '${start_date}'
    and pci.created_at < date_add('${end_date}', interval 1 day)
...
from my_bi.parcel_complaint_inquiry pci
join my_bi.abnormal_message am on json_extract(am.extra_info, '$.id') = pci.id
left join my_bi.abnormal_qaqc aq on aq.abnormal_message_id = acc.abnormal_message_id
where
    pci.created_at >= '${start_date}'
    and pci.created_at < date_add('${end_date}', interval 1 day)
    and am.punish_category = 7 -- parcel lost
...
from
    (
        ...
        from my_bi.parcel_complaint_inquiry pci
        join my_bi.parcel_claim_task pct on pct.pno = pci.merge_column
