...
#     FROM `ph_backyard`.`staff_mileage_record_attachment` smr
#     LEFT JOIN `ph_backyard`.`staff_mileage_record` sm on sm.`staff_info_id` =smr.staff_info_id and smr.mileage_date=sm.mileage_date
#     LEFT JOIN `ph_bi`.`hr_staff_info` hr on hr.`staff_info_id` =smr.`staff_info_id`
