with t as
    (
        select
            ...
            ,ftp.created_at
            ,ftp.store_id
        from my_staging.force_take_photo_record ftp
        where
            ftp.created_at > '2024-07-04 16:00:00'
            and ftp.created_at < '2024-07-05 16:00:00'
    ...
    (
        ...
        left join
            (
                ...
                where
                    pr.routed_at > '2024-07-04 16:00:00'
                    and pr.routed_at < '2024-07-05 16:00:00'
        ...
        where
            ...
            and pr.routed_at < if(fo.min_route_at is null, '2024-07-05 16:00:00', fo.min_route_at)
            ...
            (
                ...
                from
                    (
                        ...
                        from my_staging.parcel_route pr
                        join t t1 on t1.pno = pr.pno and t1.store_id = pr.store_id
                        left join dwm.dwd_dim_dict ddd on ddd.element = pr.remark and ddd.db = 'my_staging' and ddd.tablename = 'parcel_route' and ddd.fieldname = 'route_action'
                        where
                            pr.routed_at > '2024-07-04 16:00:00'
                            and pr.routed_at < '2024-07-05 16:00:00'
                            and pr.route_action = 'FORCE_TAKE_PHOTO'
...
left join
    (
        ...
        left join my_staging.parcel_route pr on pr.pno = a1.pno and pr.routed_at < a1.photo_at and pr.route_action = 'FORCE_TAKE_PHOTO'
        left join dwm.dwd_dim_dict ddd on ddd.element = pr.remark and ddd.db = 'my_staging' and ddd.tablename = 'parcel_route' and ddd.fieldname = 'route_action'
