                ...
                from
                    (
                        select
                            ...
                            ,ph.claim_at
                        from  my_staging.parcel_headless ph
                        left join my_staging.sys_store ss on ss.id=ph.submit_store_id
                        left join
                            (
                                select
                                    ph.pno
                                ...
                                from my_staging.parcel_headless ph
                                join my_bi.parcel_claim_task pct on pct.pno = ph.pno
                                where ph.state = 3 -- 时效
                                group by 1
                            ) sx on sx.pno = ph.pno
                        where ph.state < 4
            ...
            (
                ...
                from
                    (
                        ...
                        from dwm.dwd_my_sls_pro_flash_point fp
                        where
                            fp.p_date >= '2023-01-01'
                        group by 1,2
                    ) fp
                left join my_staging.sys_store ss on ss.id = fp.store_id
                where
                    ss.category in (8,12)
            ) b on ss.creat_month = b.日期 and ss.name = b.网点
