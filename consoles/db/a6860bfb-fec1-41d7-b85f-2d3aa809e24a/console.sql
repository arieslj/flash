...
left join
    (
        select
            ...
            ,bc.client_name
            ,count(vrv.id) ticket_count
            ,count(if(vrv.visit_state in (3,4,5,6,7,8) and vrv.updated_at < date_add('${date}', interval 1 day), vrv.id, null))/count(vrv.id) day_deal_rate
            ...
            ,count(if(vrv.visit_state in (3,4,7,8) and di.diff_marker_category = 17 and di.state = 0 and pi.state = 6 and vrv.type = 3, vrv.id, null )) diff_vrv_finish_count
        from nl_production.violation_return_visit vrv
        left join dwm.dwd_dim_bigClient bc on bc.client_id = vrv.client_id
