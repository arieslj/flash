select
    ...
    ,if(am.isappeal = 5 or am.isdel = 1, '已删除', '未删除')  是否申诉成功
from backyard_pro.audit_apply aa
join backyard_pro.audit_log al on aa.flow_id = al.flow_id
join backyard_pro.penalty_appeal pa on pa.id = aa.biz_value
left join bi_pro.abnormal_message am on am.id = pa.penalty_id
where
    aa.`biz_type` = 34
    and aa.`state` in (2,3)
    and aa.created_at > '2024-09-01'